...
//    private final DefaultTableModel tabModeRegistrasi;
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Riwayat Hasil Pemeriksaan LAB & RADIOLOGI ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Liberation Sans", 3, 18), new java.awt.Color(50, 50, 50))); // NOI18N
...
private void BtnPasienKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnPasienKeyPressed
    ...
    */
    ...
    private boolean isNilaiNormal(String nilai, String rujukan) {
        boolean isNormal = true;
        try {
            // Menangani kasus non-numerik terlebih dahulu
            if (nilai.toLowerCase().contains("positif") || nilai.toLowerCase().contains("abnormal") || nilai.toLowerCase().contains("reaktif")) {
                return false;
            }
            if (nilai.toLowerCase().contains("negatif") || nilai.toLowerCase().contains("normal") || nilai.toLowerCase().contains("non reaktif")) {
                return true;
            }

            // Perbandingan numerik
            double nilaiAngka = Double.parseDouble(nilai.replaceAll("[^\\d.,-]", "").replace(",", ".").trim()); 

            if (rujukan.contains("-")) {
                String[] parts = rujukan.split("-");
                double min = Double.parseDouble(parts[0].trim());
                double max = Double.parseDouble(parts[1].trim());
                if (nilaiAngka < min || nilaiAngka > max) {
                    isNormal = false;
                }
            } else if (rujukan.startsWith("<=")) {
                double max = Double.parseDouble(rujukan.replace("<=", "").trim());
                if (nilaiAngka > max) {
                    isNormal = false;
                }
            } else if (rujukan.startsWith(">=")) {
                double min = Double.parseDouble(rujukan.replace(">=", "").trim());
                if (nilaiAngka < min) {
                    isNormal = false;
                }
            } else if (rujukan.startsWith("<")) {
                            ...
                            if(rs4.next()){
                                ...
                                while(rs4.next()){
                                    try{
                                        ...
                                        s=1;
                                        while(rs2.next()){
                                            boolean hasAbnormalResult = false;
                                            StringBuilder detailLabContent = new StringBuilder();
                                            
                                            // Memeriksa hasil detail terlebih dahulu
                                            try {
                                                PreparedStatement ps3 = koneksi.prepareStatement(
                                                    ...
                                                    "detail_periksa_lab.jam='"+rs4.getString("jam")+"' order by detail_periksa_lab.kd_jenis_prw,template_laboratorium.urut ");
                                                ResultSet rs3 = ps3.executeQuery();
                                                if(rs3.next()){ 
                                                    detailLabContent.append(
                                                        "<tr>"+
                                                    ...
                                                    rs3.beforeFirst();
                                                    while(rs3.next()){
                                                        String nilai = rs3.getString("nilai");
                                                        String nilaiRujukan = rs3.getString("nilai_rujukan");
                                                        String satuan = rs3.getString("satuan");
                                                        boolean isNormal = isNilaiNormal(nilai, nilaiRujukan);
                                                        if (!isNormal) {
                                                            hasAbnormalResult = true;
                                                        }
                                                        String color = isNormal ? "black" : "red";
                                                        ...
                                                        detailLabContent.append(
                                                            "<tr>"+
                                                               ...
                                                               "<td valign='top' style='color:"+color+";'>"+nilai+" "+satuan+"</td>"+
                                                               "<td valign='top'>"+nilaiRujukan+"</td>"+
                                            ...
                                            // Menampilkan nama pemeriksaan dengan warna yang sesuai
                                            String namaPemeriksaanColor = hasAbnormalResult ? "red" : "black";
                                            if(s==1){
                                                htmlContent.append(
                                                    "<tr>"+
                                                       "<td valign='top' align='center'>"+w+"</td>"+
                                                       ...
                                                       "<td valign='top'>"+rs2.getString("kd_jenis_prw")+"</td>"+
                                                       "<td valign='top' style='color:"+namaPemeriksaanColor+";'>"+rs2.getString("nm_perawatan")+"</td>"+
                                                       "<td valign='top'>"+rs2.getString("nm_dokter")+"</td>"+
                                                       "<td valign='top'>"+rs2.getString("nama")+"</td>"+
                                                       "<td valign='top' align='right'>"+Valid.SetAngka(rs2.getDouble("biaya"))+"</td>"+
                                                    "</tr>"
                                            ...
                                            // Menambahkan detail ke konten utama
                                            htmlContent.append(detailLabContent.toString());
