                ...
                if(tbDokter.getSelectedColumn()==1){
                    ...
                    try {
                        if(!tbDokter.getValueAt(tbDokter.getSelectedRow(),8).toString().equals("")){
                            psstok=koneksi.prepareStatement("select data_batch.no_faktur,databarang."+hppfarmasi+" as dasar,ifnull(data_batch.tgl_kadaluarsa,'0000-00-00') as tgl_kadaluarsa from data_batch inner join databarang on data_batch.kode_brng=databarang.kode_brng where data_batch.no_batch=? and data_batch.kode_brng=? and data_batch.sisa>0 order by data_batch.tgl_kadaluarsa limit 1");
                            try {
        ...
        try{
            if(aktifkanbatch.equals("yes")){
                if(TCari.getText().trim().equals("")){
                    ps=koneksi.prepareStatement("select data_batch.kode_brng, databarang.nama_brng,databarang.kode_sat,databarang."+hppfarmasi+" as dasar,data_batch.no_batch,data_batch.no_faktur,  "+
        ...
        try{
            ...
            try {
                ...
                while(rs.next()){            
                    if(aktifkanbatch.equals("yes")){
                        psstok=koneksi.prepareStatement(
                                "select ifnull(gudangbarang.stok,'0'),databarang."+hppfarmasi+" as dasar,gudangbarang.no_batch,gudangbarang.no_faktur,ifnull(data_batch.tgl_kadaluarsa,'0000-00-00') as tgl_kadaluarsa "+
