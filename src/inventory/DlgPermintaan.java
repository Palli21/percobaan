...
public class DlgPermintaan extends javax.swing.JDialog {
    ...
    private int modeCariBangsal=0;
    private String[] jumlah,kodebarang,namabarang,satuan,jenis,kategori,golongan,expire,keterangan;
    ...
    public DlgPermintaan(java.awt.Frame parent, boolean modal) {
        ...
        Object[] judul={"Jml","Kode Barang","Nama Barang","Satuan","Jenis Obat","Kategori","Golongan","Kadaluarsa","Keterangan","Stok"};
...
*/
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        for(i=0;i<tbDokter.getRowCount();i++){
            String nilaiJumlah=tbDokter.getValueAt(i,0)==null?"":tbDokter.getValueAt(i,0).toString();
            if(Valid.SetAngka(nilaiJumlah)>0){
                jml++;
            }
        }
                ...
                if(Sequel.menyimpantf2("permintaan_medis","?,?,?,?,?,?","No.Permintaan",6,new String[]{
                        ...
                        for(i=0;i<jml;i++){  
                            try {
                                String nilaiJumlah=tbDokter.getValueAt(i,0)==null?"":tbDokter.getValueAt(i,0).toString();
                                if(Valid.SetAngka(nilaiJumlah)>0){
                                    String keteranganItem=tbDokter.getValueAt(i,8)==null?"":tbDokter.getValueAt(i,8).toString();
                                    if(Sequel.menyimpantf2("detail_permintaan_medis","?,?,?,?,?","Detail Permintaan",5,new String[]{
                                           ...
                                           nilaiJumlah,
                                           keteranganItem.replaceAll("'","").replaceAll("\"","")
                                    })==false){
                                        sukses=false;
                                    }else{
                                        detailTersimpan++;
                        ...
                        if(detailTersimpan==0){
                            sukses=false;
                            JOptionPane.showMessageDialog(null,"Tidak ada detail permintaan dengan jumlah lebih dari 0.");
                        }
                }else{
                    sukses=false;
...
private void tbDokterKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbDokterKeyPressed
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        try {
            File cacheFile=new File("./cache/permintaanobat.iyem");
            if(!cacheFile.exists()||Valid.daysOld(cacheFile.getPath())>=8){
                tampil();
            }
            tampil2();
        } catch (Exception e) {
    ...
    */
    ...
    private void tampil() {
        StringBuilder isiCache=new StringBuilder();
        try{
            File file=new File("./cache/permintaanobat.iyem");
            file.createNewFile();
            ps=koneksi.prepareStatement(
                "select databarang.kode_brng,databarang.nama_brng,databarang.kode_sat,jenis.nama,"+
                "kategori_barang.nama as kategori,golongan_barang.nama as golongan,ifnull(databarang.expire,'') as kadaluarsa "+
            ...
            try {
                rs=ps.executeQuery();
                while(rs.next()){
                    if(isiCache.length()>0){
                        isiCache.append(",");
                    }
                    isiCache.append("{\"KodeBarang\":\"").append(rs.getString(1))
                           ...
                           .append("\",\"Kadaluarsa\":\"").append(rs.getString(7))
                           .append("\"}");
                } 
            } catch (Exception e) {
                System.out.println("Notifikasi : "+e);
            ...
            try(FileWriter fileWriter = new FileWriter(file)){
                fileWriter.write("{\"permintaanobat\":["+isiCache+"]}");
                fileWriter.flush();
            }
        }catch(Exception e){
            System.out.println("Notifikasi : "+e);
    ...
    private void tampil2() {
        try{
            ...
            expire=null;
            expire=new String[jml];
            keterangan=null;
            keterangan=new String[jml];
            String gudangTujuan=kdgudangTujuan.getText().trim();
            boolean filterStok=!gudangTujuan.equals("");
            HashMap<String,Double> stokPerBarang=ambilStokPerBarang(gudangTujuan);
            HashSet<String> kodeSudahDitampilkan=new HashSet<>();
            index=0;        
            for(i=0;i<row;i++){
                String nilaiJumlah=tbDokter.getValueAt(i,0)==null?"":tbDokter.getValueAt(i,0).toString();
                if(!nilaiJumlah.equals("")){
                    jumlah[index]=nilaiJumlah;
                    kodebarang[index]=tbDokter.getValueAt(i,1)==null?"":tbDokter.getValueAt(i,1).toString();
                    namabarang[index]=tbDokter.getValueAt(i,2)==null?"":tbDokter.getValueAt(i,2).toString();
                    satuan[index]=tbDokter.getValueAt(i,3)==null?"":tbDokter.getValueAt(i,3).toString();
                    jenis[index]=tbDokter.getValueAt(i,4)==null?"":tbDokter.getValueAt(i,4).toString();
                    kategori[index]=tbDokter.getValueAt(i,5)==null?"":tbDokter.getValueAt(i,5).toString();
                    golongan[index]=tbDokter.getValueAt(i,6)==null?"":tbDokter.getValueAt(i,6).toString();
                    expire[index]=tbDokter.getValueAt(i,7)==null?"":tbDokter.getValueAt(i,7).toString();
                    keterangan[index]=tbDokter.getValueAt(i,8)==null?"":tbDokter.getValueAt(i,8).toString();
                    index++;
                }
            }
            Valid.tabelKosong(tabMode);
            for(i=0;i<jml;i++){
                String kd=kodebarang[i];
                if(kd.equals("")||kodeSudahDitampilkan.contains(kd)){
                    continue;
                }
                double stokBarang=getStokBarang(kd,stokPerBarang,filterStok);
                if(!filterStok || stokBarang>0){
                    tabMode.addRow(new Object[]{jumlah[i],kodebarang[i],namabarang[i],satuan[i],jenis[i],kategori[i],golongan[i],expire[i],keterangan[i],filterStok?Valid.SetAngka2(stokBarang):""});
                    kodeSudahDitampilkan.add(kd);
                }
            }
            
            try(FileReader myObj = new FileReader("./cache/permintaanobat.iyem")){
                root = mapper.readTree(myObj);
            }
            response = root.path("permintaanobat");
            String keyword=TCari.getText().trim().toLowerCase();
            HashMap<String,String> cacheKadaluarsaKosong=new HashMap<>();
            if(response.isArray()){
                for(JsonNode list:response){
                    ...
                    if(!keyword.equals("")){
                        if(!list.path("KodeBarang").asText().toLowerCase().contains(keyword) &&
                           !list.path("NamaBarang").asText().toLowerCase().contains(keyword) &&
                           !list.path("JenisObat").asText().toLowerCase().contains(keyword) &&
                           !list.path("Kategori").asText().toLowerCase().contains(keyword) &&
                           !list.path("Golongan").asText().toLowerCase().contains(keyword)){
                            continue;
                        }
                    }

                    String kadaluarsa=list.path("Kadaluarsa").asText();
                    if(kadaluarsa.equals("")){
                        kadaluarsa=cacheKadaluarsaKosong.get(kd);
                        if(kadaluarsa==null){
                            kadaluarsa=Sequel.cariIsi("select ifnull(expire,'') from databarang where kode_brng=?",kd);
                            cacheKadaluarsaKosong.put(kd,kadaluarsa);
                        }
                    }
                    ...
                    if(!filterStok || stokBarang>0){
                        tabMode.addRow(new Object[]{
                            "",kd,list.path("NamaBarang").asText(),list.path("Satuan").asText(),list.path("JenisObat").asText(),list.path("Kategori").asText(),list.path("Golongan").asText(),kadaluarsa,"",filterStok?Valid.SetAngka2(stokBarang):""
                        });
                        kodeSudahDitampilkan.add(kd);
                    }
                }
    ...
    private HashMap<String,Double> ambilStokPerBarang(String gudang){
        HashMap<String,Double> stokPerBarang=new HashMap<>();
        if(gudang.equals("")){
            return stokPerBarang;
        }

        try(PreparedStatement psStok=koneksi.prepareStatement(
                "select kode_brng,ifnull(sum(stok),0) as stok from gudangbarang where kd_bangsal=? group by kode_brng")){
            psStok.setString(1,gudang);
            try(ResultSet rsStok=psStok.executeQuery()){
                while(rsStok.next()){
                    stokPerBarang.put(rsStok.getString("kode_brng"),rsStok.getDouble("stok"));
                }
            }
        } catch (Exception e) {
            System.out.println("Notifikasi : "+e);
    ...
    private double getStokBarang(String kodeBarang,HashMap<String,Double> stokPerBarang,boolean filterStok){
        if(!filterStok){
            return 0;
        }
        Double stok=stokPerBarang.get(kodeBarang);
        return stok==null?0:stok;
    }
