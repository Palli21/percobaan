...
*/
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        if(TabPilihRawat.getSelectedIndex()==0){
            if(TabRawatJalan.getSelectedIndex()==0){
                if(akses.getresep_dokter()==false){
                    JOptionPane.showMessageDialog(null,"Maaf, Anda tidak punya hak akses untuk mengubah...!!!!");
                    TCari.requestFocus();
                }else if(tabMode.getRowCount()==0){
                    JOptionPane.showMessageDialog(null,"Maaf, data sudah habis...!!!!");
                    TCari.requestFocus();
            ...
            if(TabRawatInap.getSelectedIndex()==0){
                ...
                if(akses.getpermintaan_stok_obat_pasien()==false){
                    JOptionPane.showMessageDialog(null,"Maaf, Anda tidak punya hak akses untuk mengubah...!!!!");
                    TCari.requestFocus();
                }else if(tabMode5.getRowCount()==0){
                    JOptionPane.showMessageDialog(null,"Maaf, data sudah habis...!!!!");
                ...
                if(akses.getpermintaan_resep_pulang()==false){
                    JOptionPane.showMessageDialog(null,"Maaf, Anda tidak punya hak akses untuk mengubah...!!!!");
                    TCari.requestFocus();
                }else if(tabMode7.getRowCount()==0){
                    JOptionPane.showMessageDialog(null,"Maaf, data sudah habis...!!!!");
        ...
        if(TabPilihRawat.getSelectedIndex()==0){
            if(TabRawatJalan.getSelectedIndex()==0){
                if(akses.getresep_dokter()==true){
                    ...
                    JOptionPane.showMessageDialog(null,"Maaf, Anda tidak punya hak akses untuk menghapus...!!!!");
                    TCari.requestFocus();
                }
            }else if(TabRawatJalan.getSelectedIndex()==1){
                JOptionPane.showMessageDialog(null,"Maaf, silahkan buka Daftar Resep...!!!!");
        ...
        if(TabPilihRawat.getSelectedIndex()==0){
            if(TabRawatJalan.getSelectedIndex()==0){
                if(tabMode.getRowCount()==0){
                    ...
                    param.put("alamat",Sequel.cariIsi("select alamat from pasien where no_rkm_medis=?",NoRM));
                    param.put("lahir",Valid.SetTgl3(Sequel.cariIsi("select tgl_lahir from pasien where no_rkm_medis=?",NoRM)));
                    param.put("berat",Sequel.cariIsi("select berat from pemeriksaan_ralan where no_rawat=?",NoRawat));
            ...
            if(TabRawatInap.getSelectedIndex()==0){
                if(tabMode3.getRowCount()==0){
                    ...
                    param.put("alamat",Sequel.cariIsi("select alamat from pasien where no_rkm_medis=?",NoRM));
                    param.put("lahir",Valid.SetTgl3(Sequel.cariIsi("select tgl_lahir from pasien where no_rkm_medis=?",NoRM)));
                    param.put("berat",Sequel.cariIsi("select berat from pemeriksaan_ranap where no_rawat=?",NoRawat));
                    finger=Sequel.cariIsi("select sha1(sidikjari.sidikjari) from sidikjari inner join pegawai on pegawai.id=sidikjari.id where pegawai.nik=?",KodeDokter);
                    param.put("finger","Dikeluarkan di "+akses.getnamars()+", Kabupaten/Kota "+akses.getkabupatenrs()+"\nDitandatangani secara elektronik oleh "+DokterPeresep+"\nID "+(finger.equals("")?KodeDokter:finger)+"\n"+Valid.SetTgl3(TglPeresepan));
                    param.put("jam",JamPeresepan);
                    param.put("logo",Sequel.cariGambar("select setting.logo from setting"));
        ...
        try{  
            ...
            if(DEPOAKTIFOBAT.equals("")){
                ps=koneksi.prepareStatement("select resep_obat.no_resep,resep_obat.tgl_peresepan,resep_obat.jam_peresepan,"+
                        ...
                        "dokter.nm_dokter like ? or penjab.png_jawab like ?)")+" order by (resep_obat.tgl_perawatan='0000-00-00') desc,resep_obat.tgl_peresepan desc,resep_obat.jam_peresepan desc");
            }else{
                ps=koneksi.prepareStatement("select resep_obat.no_resep,resep_obat.tgl_peresepan,resep_obat.jam_peresepan,"+
                        " resep_obat.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,resep_obat.kd_dokter,dokter.nm_dokter,"+
                        " if(resep_obat.tgl_perawatan='0000-00-00','Belum Terlayani','Sudah Terlayani') as status,poliklinik.nm_poli, "+
    ...
    private void setAksesBtnEdit(){
        if(TabPilihRawat.getSelectedIndex()==0){
            ...
            if(TabRawatInap.getSelectedIndex()==0){
                ...
                BtnEdit.setEnabled(akses.getpermintaan_stok_obat_pasien());
                BtnTambah.setEnabled(akses.getstok_obat_pasien());
                BtnHapus.setEnabled(akses.getpermintaan_stok_obat_pasien());
            }else if(TabRawatInap.getSelectedIndex()==4){
                BtnEdit.setEnabled(akses.getpermintaan_resep_pulang());
                BtnTambah.setEnabled(akses.getpermintaan_resep_pulang());
        ...
        try{  
            ...
            if(DEPOAKTIFOBAT.equals("")){
                ps=koneksi.prepareStatement("select resep_obat.no_resep,resep_obat.tgl_peresepan,resep_obat.jam_peresepan,"+
                        ...
                        " group by resep_obat.no_resep order by (resep_obat.tgl_perawatan='0000-00-00') desc,resep_obat.tgl_peresepan desc,resep_obat.jam_peresepan desc");
            }else{
                ps=koneksi.prepareStatement("select resep_obat.no_resep,resep_obat.tgl_peresepan,resep_obat.jam_peresepan,"+
                        " resep_obat.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,resep_obat.kd_dokter,dokter.nm_dokter, "+
                        " if(resep_obat.tgl_perawatan='0000-00-00','Belum Terlayani','Sudah Terlayani') as status,"+
        ...
        try{  
            ...
            if(DEPOAKTIFOBAT.equals("")){
                ps=koneksi.prepareStatement("select permintaan_stok_obat_pasien.no_permintaan,permintaan_stok_obat_pasien.tgl_permintaan,permintaan_stok_obat_pasien.jam,"+
                        ...
                        "pasien.no_rkm_medis like ? or pasien.nm_pasien like ? or dokter.nm_dokter like ? or penjab.png_jawab like ?)")+
                        " group by permintaan_stok_obat_pasien.no_permintaan order by (permintaan_stok_obat_pasien.status='Belum') desc,permintaan_stok_obat_pasien.tgl_permintaan desc,permintaan_stok_obat_pasien.jam desc");
            }else{
                ps=koneksi.prepareStatement("select permintaan_stok_obat_pasien.no_permintaan,permintaan_stok_obat_pasien.tgl_permintaan,permintaan_stok_obat_pasien.jam,"+
                        " permintaan_stok_obat_pasien.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,permintaan_stok_obat_pasien.kd_dokter,dokter.nm_dokter, "+
                        " if(permintaan_stok_obat_pasien.status='Belum','Belum Terlayani','Sudah Terlayani') as status,"+
        ...
        try{  
            ...
            if(DEPOAKTIFOBAT.equals("")){
                ps=koneksi.prepareStatement("select permintaan_resep_pulang.no_permintaan,permintaan_resep_pulang.tgl_permintaan,permintaan_resep_pulang.jam,"+
                        ...
                        "pasien.no_rkm_medis like ? or pasien.nm_pasien like ? or dokter.nm_dokter like ? or penjab.png_jawab like ?)")+
                        " group by permintaan_resep_pulang.no_permintaan order by (permintaan_resep_pulang.status='Belum') desc,permintaan_resep_pulang.tgl_permintaan desc,permintaan_resep_pulang.jam desc");
            }else{
                ps=koneksi.prepareStatement("select permintaan_resep_pulang.no_permintaan,permintaan_resep_pulang.tgl_permintaan,permintaan_resep_pulang.jam,"+
                        " permintaan_resep_pulang.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,permintaan_resep_pulang.kd_dokter,dokter.nm_dokter, "+
                        " if(permintaan_resep_pulang.status='Belum','Belum Terlayani','Sudah Terlayani') as status,"+
