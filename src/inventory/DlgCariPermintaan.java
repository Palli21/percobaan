...
public class DlgCariPermintaan extends javax.swing.JDialog {
    ...
    public DlgCariPermintaan(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"Tanggal","No.Permintaan","Asal Permintaan","Pegawai","Ditujukan Ke","Stok"};
...
*/
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            for(int i=0;i<row;i++){  
                String[] tmpPrint=new String[38];
                tmpPrint[0]=String.valueOf(i);
                tmpPrint[1]=tabMode.getValueAt(i,0).toString();
                tmpPrint[2]=tabMode.getValueAt(i,1).toString();
                tmpPrint[3]=tabMode.getValueAt(i,2).toString();
                tmpPrint[4]=tabMode.getValueAt(i,3).toString();
                tmpPrint[5]=tabMode.getValueAt(i,4).toString();
                tmpPrint[6]=tabMode.getValueAt(i,5).toString();
                for(int j=7;j<37;j++){
                    tmpPrint[j]="";
                }
                tmpPrint[37]=akses.getalamatip();
                Sequel.menyimpan("temporary","'"+String.join("','",tmpPrint)+"'","Transaksi Pembelian"); 
            }
       ...
       int permintaanDitampilkan=0;
        try{  
            ...
            try {
                ...
                rs=ps.executeQuery();
                while(rs.next()){
                    boolean headerDitambahkan=false;
                    ...
                    try {
                        ...
                        int no=1;
                        while(rs2.next()){
                            double stok=Sequel.cariIsiAngka2("select ifnull(sum(stok),0) from gudangbarang where kd_bangsal=? and kode_brng=?",rs.getString("kd_bangsaltujuan"),rs2.getString("kode_brng"));
                            if(stok>0){
                                if(!headerDitambahkan){
                                    ...
                                    headerDitambahkan=true;
                                    permintaanDitampilkan++;
                                }
                                tabMode.addRow(new Object[]{
                                    "",no+". "+rs2.getString("kode_brng")+" "+rs2.getString("nama_brng"),
                                    rs2.getString("jumlah"),rs2.getString("satuan"),rs2.getString("keterangan"),Valid.SetAngka2(stok)
                                });
                                no++;
                            }
                        } 
                ...
                LTotal.setText(String.valueOf(permintaanDitampilkan));
            } catch (Exception e) {
