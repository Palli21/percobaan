...
public class DlgCariPeriksaRadiologi extends javax.swing.JDialog {
    ...
    private static final String BASE_FRAME_TITLE = "::[ Data Pemeriksaan Radiologi ]::";
    ...
    private void initComponents() {
        ...
        btnPetugas.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/bawahpaper.png"))); // NOI18N
        ...
        btnAmbilPhoto.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/bawahtambah1.png"))); // NOI18N
        ...
        BtnRefreshPhoto.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/BawahRecaycel_9_11zon.png"))); // NOI18N
        ...
        labelNoHpWA.setText("No. HP Tujuan :");
        labelNoHpWA.setName("labelNoHpWA"); // NOI18N
        labelNoHpWA.setPreferredSize(new java.awt.Dimension(110, 23));
        FormPass2.add(labelNoHpWA);

        NoHpWA.setName("NoHpWA"); // NOI18N
        NoHpWA.setPreferredSize(new java.awt.Dimension(140, 23));
        NoHpWA.setToolTipText("Isi untuk override nomor tujuan WA");
        FormPass2.add(NoHpWA);
        ...
        btnAmbilPhoto1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/bawahpaper.png"))); // NOI18N
        ...
        BtnPrint1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/bawahprint.png"))); // NOI18N
...
private void tbDokterKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbDokterKeyPressed
   if(tabMode.getRowCount()!=0){
    ...
    private String buildRadiologiWaMessage(String nama, String noRm, String noRawat, String dokterPerujuk, String hasil) {
        ...
        String ruangan = resolveRadiologiRuangan(noRawat);
        String hasilSafe = safeTrim(hasil);
        if (hasilSafe.isEmpty()) {
            hasilSafe = "-";
        }
    ...
    private String resolveRadiologiRuangan(String noRawat) {
    ...
    private String resolveRadiologiImagePath(String lokasiGambar) {
        String lokasi = safeTrim(lokasiGambar);
        if (lokasi.isEmpty()) {
            return "";
        }
        String normalized = lokasi.replace("\\", "/");
        String lower = normalized.toLowerCase();
        if (lower.startsWith("http://") || lower.startsWith("https://")) {
            int idx = lower.indexOf("/radiologi/");
            if (idx >= 0) {
                normalized = normalized.substring(idx + "/radiologi/".length());
            } else {
                idx = lower.indexOf("/webapps/");
                if (idx >= 0) {
                    normalized = normalized.substring(idx + "/webapps/".length());
                    if (normalized.startsWith("radiologi/")) {
                        normalized = normalized.substring("radiologi/".length());
                    }
                } else {
                    return "";
                }
        ...
        File file = new File("webapps/radiologi", normalized);
    ...
    private String buildRadiologiImageUrl(String lokasiGambar) {
        String lokasi = safeTrim(lokasiGambar);
        if (lokasi.isEmpty()) {
            return "";
        }
        String lower = lokasi.toLowerCase();
        if (lower.startsWith("http://") || lower.startsWith("https://")) {
            return lokasi;
        }
        String normalized = lokasi.replace("\\", "/");
        ...
        String host = safeTrim(koneksiDB.HOSTHYBRIDWEB());
        String port = safeTrim(koneksiDB.PORTWEB());
        String base = safeTrim(koneksiDB.HYBRIDWEB());
        if (host.isEmpty() || base.isEmpty()) {
            return koneksiDBSalim.URLWEBAPPS() + "radiologi/" + normalized;
        }
        ...
        url.append("/").append(base).append("/radiologi/").append(normalized);
        return url.toString();
    }

    private String downloadRadiologiImage(String url, String noRawat) {
        String targetUrl = safeTrim(url);
        if (targetUrl.isEmpty()) {
            return "";
        }
        try {
            File outDir = new File("tmpImageFreehand");
            if (!outDir.exists()) {
                outDir.mkdirs();
            }
            String safeNoRawat = safeTrim(noRawat).replaceAll("[^a-zA-Z0-9_-]", "");
            if (safeNoRawat.isEmpty()) {
                safeNoRawat = "radiologi";
            }
            String fileName = "radimg_" + safeNoRawat + "_" + System.currentTimeMillis() + ".jpg";
            ...
            try (InputStream in = con.getInputStream(); FileOutputStream fos = new FileOutputStream(outFile)) {
    ...
    private String createRadiologiPdfFromImage(String imagePath, String noRawat) {
        ...
        String fileName = "radimg_" + safeNoRawat + "_" + System.currentTimeMillis();
        String pdfPath = new File(outDir, fileName + ".pdf").getPath();
        PDDocument doc = null;
        try {
            doc = new PDDocument();
            ...
            content.drawImage(pdImage, 0, 0, rect.getWidth(), rect.getHeight());
            content.close();
            doc.save(pdfPath);
            return pdfPath;
        } catch (Exception e) {
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        try {
                            ...
                            String noWa = resolveManualWaTarget(rs.getString("no_tlp"));
                            String countryCode = "62";
                            String pesan = buildRadiologiWaMessage(
                            ...
                            String urlHasil = buildRadiologiImageUrl(rs.getString("lokasi_gambar"));
                            String imagePath = resolveRadiologiImagePath(rs.getString("lokasi_gambar"));

                            if ("waha".equalsIgnoreCase(waGateway)) {
                                WahaClient.Result result;
                                String pesanKirim = urlHasil.trim().isEmpty() ? pesan : pesan + "\n\n" + urlHasil;
                                if (!urlHasil.trim().isEmpty()) {
                                    result = WahaClient.sendFile(noWa, urlHasil, pesan);
                                    if (!result.ok) {
                                        result = WahaClient.sendText(noWa, pesanKirim);
                                    }
                                } else {
                                ...
                                if (!result.ok) {
                                    ...
                                    Sequel.queryu("INSERT INTO wa_report VALUES ('" + result.messageId + "', '" + rs.getString("no_rkm_medis") + "', '" + noWa + "', '" + akses.getkode() + "', 'Kirim Hasil Radilogi Pasien', '" + result.chatId + "', '" + simpanPesan + "', '', 'true', '', '" + dateNow + "', '" + dateNow + "')");
                                    JOptionPane.showMessageDialog(null, "Berhasil Mengirim Pesan !!", "Success", JOptionPane.INFORMATION_MESSAGE);
                                }
                            } else if ("golang".equalsIgnoreCase(waGateway) || "wagolang".equalsIgnoreCase(waGateway)) {
                                WagolangClient.Result result = null;
                                String tempImagePath = "";
                                if (imagePath.isEmpty()) {
                                    tempImagePath = downloadRadiologiImage(urlHasil, rs.getString("no_rawat"));
                                    if (!tempImagePath.isEmpty()) {
                                        imagePath = tempImagePath;
                                    }
                                }
                                String pdfPath = createRadiologiPdfFromImage(imagePath, rs.getString("no_rawat"));
                                try {
                                    if (!pdfPath.isEmpty()) {
                                        result = WagolangClient.sendFileMultipartWithKey(noWa, pdfPath, pesan, "file");
                                    } else {
                                        JOptionPane.showMessageDialog(null,
                                            "File gambar tidak ditemukan. lokasi_gambar=" + rs.getString("lokasi_gambar") +
                                            "\nimagePath=" + imagePath +
                                            "\nurlHasil=" + urlHasil,
                                    ...
                                    if (!result.ok) {
                                        System.out.println("WA Wagolang error: " + result.error + " body=" + result.responseBody);
                                        JOptionPane.showMessageDialog(null,
                                            "Wagolang response:\n" + result.responseBody,
                                            "Wagolang Error", JOptionPane.ERROR_MESSAGE);
                                        result = WagolangClient.sendText(noWa, pesan);
                                    }
                                } finally {
                                    if (!tempImagePath.isEmpty()) {
                                    ...
                                    if (!pdfPath.isEmpty() && result != null && result.ok) {
                                ...
                                Connection tokenWA = koneksiDBWa.condb();
...
//                            con.setRequestProperty("Authorization", tokenWA);
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        try {
                            ...
                            String countryCode = "62";
                            String noWa = resolveManualWaTarget(Sequel.cariIsi("select no_telp from dokter where nm_dokter=?", tbDokter.getValueAt(tbDokter.getSelectedRow(), 5).toString()));
                            String pesan = buildRadiologiWaMessage(
                                    rs.getString("nm_pasien"),
                                    rs.getString("no_rkm_medis"),
                                    rs.getString("no_rawat"),
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        try {
                            ...
                            String tokenWA = koneksiDBSalim.TOKENWA();
                            String countryCode = "62";
                            String noWa = resolveManualWaTarget(Sequel.cariIsi("select no_telp from dokter where nm_dokter=?", tbDokter.getValueAt(tbDokter.getSelectedRow(), 6).toString()));
                            String pesan = buildRadiologiWaMessage(
                                    rs.getString("nm_pasien"),
                                    rs.getString("no_rkm_medis"),
                                    rs.getString("no_rawat"),
    ...
    */
    ...
    private widget.Label labelNoHpWA;
