...
public class DlgCariPeriksaLab extends javax.swing.JDialog {
    ...
    private static final int DEFAULT_LAB_PDF_MODEL = 1;
    ...
    public DlgCariPeriksaLab(java.awt.Frame parent, boolean modal) {
        ...
        NoWAManual.setDocument(new batasInput((byte)20).getOnlyAngka(NoWAManual));
    ...
    private void initComponents() {
        ...
        MnKirimWhatsapp = new javax.swing.JMenuItem();
        ...
        labelNoWAManual = new widget.Label();
        NoWAManual = new widget.TextBox();
        ...
        labelNoWAManual.setText("No.WA Manual :");
        labelNoWAManual.setName("labelNoWAManual"); // NOI18N
        labelNoWAManual.setPreferredSize(new java.awt.Dimension(90, 23));
        panelisi3.add(labelNoWAManual);
        labelNoWAManual.setBounds(808, 10, 90, 23);

        NoWAManual.setName("NoWAManual"); // NOI18N
        NoWAManual.setPreferredSize(new java.awt.Dimension(120, 23));
        panelisi3.add(NoWAManual);
        NoWAManual.setBounds(900, 10, 140, 23);
        ...
        BtnKirimWA.setMnemonic('W');
        BtnKirimWA.setText("Kirim WA");
        BtnKirimWA.setToolTipText("Alt+W");
        BtnKirimWA.setName("BtnKirimWA"); // NOI18N
        BtnKirimWA.setPreferredSize(new java.awt.Dimension(110, 30));
...
*/
    ...
    private void BtnKirimWAKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnKirimWAKeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_SPACE){
            ...
            Valid.pindah(evt,BtnKirimWA,BtnKeluar);
        }
    ...
    private void BtnKeluarKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnKeluarKeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_SPACE){
            dispose();
        }else{Valid.pindah(evt,BtnKirimWA,NoRawat);}
...
private void tbDokterKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbDokterKeyPressed
   if(tabMode.getRowCount()!=0){
    ...
    private void MnKirimWhatsappActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnKirimWhatsappActionPerformed
        int[] selectedRows = tbDokter.getSelectedRows();
        if (selectedRows == null || selectedRows.length == 0) {
            JOptionPane.showMessageDialog(null, "Maaf, silahkan pilih data..!!");
            return;
        }

        String waGateway = safeTrim(koneksiDB.WAGATEWAY());
        ...
        Set<Integer> masterRows = new LinkedHashSet<>();
        for (int row : selectedRows) {
            int master = resolveMasterRowIndexFrom(row);
            if (master >= 0) {
                masterRows.add(master);
            }
        }
        if (masterRows.isEmpty()) {
            JOptionPane.showMessageDialog(null, "Maaf, silahkan pilih data pemeriksaan utama..!!");
            return;
        }

        boolean bulk = masterRows.size() > 1;
        int sukses = 0;
        int gagal = 0;
        int dilewati = 0;
        ...
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        try {
            for (int row : masterRows) {
                boolean pdfReady = false;
                try {
                    tbDokter.setRowSelectionInterval(row, row);
                    String noRawat = tbDokter.getValueAt(row, 0).toString().trim();
                    String tglPeriksa = tbDokter.getValueAt(row, 3).toString().trim();
                    String jamPeriksa = tbDokter.getValueAt(row, 4).toString().trim();
                    if (noRawat.isEmpty()) {
                        gagal++;
                        continue;
                    }
                    ...
                    ps = koneksi.prepareStatement(
                        "select pasien.no_rkm_medis,pasien.nm_pasien,pasien.no_tlp,periksa_lab.dokter_perujuk,periksa_lab.kd_dokter,dokter.nm_dokter " +
                        ...
                        "where periksa_lab.kategori='PK' and periksa_lab.no_rawat=? and periksa_lab.tgl_periksa=? and periksa_lab.jam=? limit 1"
                    );
                    ...
                    if (!rs.next()) {
                        if (!bulk) {
                            JOptionPane.showMessageDialog(null, "Data pemeriksaan tidak ditemukan.");
                        }
                        gagal++;
                        continue;
                    }
                    ...
                    String namaPasien = safeTrim(rs.getString("nm_pasien"));
                    String noTlp = safeTrim(rs.getString("no_tlp"));
                    String dokterPerujuk = Sequel.cariIsi("select dokter.nm_dokter from dokter where dokter.kd_dokter=?", rs.getString("dokter_perujuk"));
                    String dokterPj = safeTrim(rs.getString("nm_dokter"));

                    String noWa = resolveManualWaTarget(noTlp);
                    if (noWa.isEmpty()) {
                        if (!bulk) {
                    ...
                    String fileName = buildLabFileName(noRawat, noRm, tglPeriksa, jamPeriksa);
                    Kd2.setText(noRawat);
                    CreatePDF(fileName, DEFAULT_LAB_PDF_MODEL);
                    pdfReady = true;

                    File pdfFile = new File("tmpPDF/" + fileName + ".pdf");
                    if (!pdfFile.exists()) {
                        if (!bulk) {
                            JOptionPane.showMessageDialog(null, "File PDF hasil lab tidak ditemukan.");
                        }
                        gagal++;
                        continue;
                    }

                    String pesan = buildLabWaMessage(namaPasien, noRm, noRawat, tglPeriksa, jamPeriksa, dokterPerujuk, dokterPj);
                    ...
                    if (useWaha) {
                        ...
                        if (!uploadPDFToHybrid(fileName, docPath, !bulk)) {
                            gagal++;
                            continue;
                        }
                        String fileUrl = buildLabFileUrl(fileName, docPath);
                        if (fileUrl.isEmpty()) {
                            if (!bulk) {
                        ...
                        WagolangClient.Result result = WagolangClient.sendFileMultipart(noWa, pdfFile.getPath(), pesan);
                        if (!result.ok) {
                    ...
                    if (!terkirim) {
                        ...
                        Sequel.queryu("INSERT INTO wa_report VALUES ('" + messageId + "', '" + noRm + "', '" + noWa + "', '" + akses.getkode() + "', 'Kirim Hasil Laboratorium Pasien', '" + chatId + "', '" + simpanPesan + "', '', 'true', '', '" + dateNow + "', '" + dateNow + "')");
                        if (!bulk) {
                    ...
                    System.out.println("Notif kirim WA lab : " + e);
                    gagal++;
                } finally {
                    try {
                        if (rs != null) {
                        ...
                        System.out.println("Notif close rs WA lab : " + e);
                    }
                    try {
                        if (ps != null) {
                            ps.close();
                        ...
                        System.out.println("Notif close ps WA lab : " + e);
                    }
                    if (pdfReady) {
                        HapusPDF();
                    }
        ...
        if (bulk) {
            JOptionPane.showMessageDialog(null,
                    "Selesai kirim WA Hasil Lab.\nBerhasil: " + sukses + "\nGagal: " + gagal + "\nDilewati (No. HP kosong): " + dilewati,
                    "Informasi", JOptionPane.INFORMATION_MESSAGE);
        }
    }//GEN-LAST:event_MnKirimWhatsappActionPerformed
    ...
    private void UploadPDFActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_UploadPDFActionPerformed
        if (tbDokter.getSelectedRow() == -1) {
            JOptionPane.showMessageDialog(null, "Silakan pilih data Surat PRI yang akan diproses.");
            return;
        }

        // Ambil no_rawat dari kolom 0 (bukan 1)
        String noRawat = tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim();

        // Cek apakah no_rawat valid di reg_periksa
        if (Sequel.cariInteger("SELECT COUNT(no_rawat) FROM reg_periksa WHERE no_rawat=?", noRawat) == 0) {
            JOptionPane.showMessageDialog(null, "Gagal membuat SPRI: No. Rawat tidak ditemukan di reg_periksa!", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        // Format nama file
        FileName = "LAB_"
                + tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().replaceAll("/", "") // no_rawat
                + "_" + tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString() // no_kartu
                + "_" + tbDokter.getValueAt(tbDokter.getSelectedRow(), 3).toString();              // nama pasien

        CreatePDF(FileName, DEFAULT_LAB_PDF_MODEL);
        uploadPDFToHybrid(FileName, "pages/upload/", true);  // Pastikan docpath ini sesuai dengan upload.php
        HapusPDF();
    ...
    */
    ...
    private javax.swing.JMenuItem MnKirimWhatsapp;
    ...
    private widget.TextBox NoWAManual;
    ...
    private widget.Label labelNoWAManual;
    ...
    public void isCek(){
        ...
        MnKirimWhatsapp.setEnabled(akses.getperiksa_lab());
   ...
   private void CreatePDF(String FileName, int model) {
       this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        if(tabMode.getRowCount()==0){
            JOptionPane.showMessageDialog(null,"Maaf, data sudah habis...!!!!");
            TCari.requestFocus();
            ...
            try {   
                int resolvedModel = (model < 1 || model > 12) ? DEFAULT_LAB_PDF_MODEL : model;
                String reportName = "rptPeriksaLab.jasper";
                String reportPermintaan = "rptPeriksaLabPermintaan.jasper";
                switch (resolvedModel) {
                    case 2:
                        reportName = "rptPeriksaLab2.jasper";
                        reportPermintaan = "rptPeriksaLab2Permintaan.jasper";
                        break;
                    case 3:
                        reportName = "rptPeriksaLab3.jasper";
                        reportPermintaan = "rptPeriksaLab3Permintaan.jasper";
                        break;
                    case 4:
                        reportName = "rptPeriksaLab4.jasper";
                        reportPermintaan = "rptPeriksaLab4Permintaan.jasper";
                        break;
                    case 5:
                        reportName = "rptPeriksaLab5.jasper";
                        reportPermintaan = "rptPeriksaLab5Permintaan.jasper";
                        break;
                    case 6:
                        reportName = "rptPeriksaLab6.jasper";
                        reportPermintaan = "rptPeriksaLab6Permintaan.jasper";
                        break;
                    case 7:
                        reportName = "rptPeriksaLab7.jasper";
                        reportPermintaan = "rptPeriksaLab7Permintaan.jasper";
                        break;
                    case 8:
                        reportName = "rptPeriksaLab8.jasper";
                        reportPermintaan = "rptPeriksaLab8Permintaan.jasper";
                        break;
                    case 9:
                        reportName = "rptPeriksaLab9.jasper";
                        reportPermintaan = "rptPeriksaLab9Permintaan.jasper";
                        break;
                    case 10:
                        reportName = "rptPeriksaLab10.jasper";
                        reportPermintaan = "rptPeriksaLab10Permintaan.jasper";
                        break;
                    case 11:
                        reportName = "rptPeriksaLab11.jasper";
                        reportPermintaan = "rptPeriksaLab11Permintaan.jasper";
                        break;
                    case 12:
                        reportName = "rptPeriksaLab12.jasper";
                        reportPermintaan = "rptPeriksaLab12Permintaan.jasper";
                        break;
                    default:
                        break;
                }
                    ...
                    while(rs.next()){
                        ...
                        try {
                            ...
                            if(rspermintaan.next()){
                                ...
                                param.put("tanggalpermintaan",rspermintaan.getString("tgl_permintaan"));  
                                param.put("jampermintaan",rspermintaan.getString("jam_permintaan"));
                                Valid.MyReportPDFUpload(reportPermintaan,"report","::[ Pemeriksaan Laboratorium ]::", FileName, param);
                            }else{
                                Valid.MyReportPDFUpload(reportName,"report","::[ Pemeriksaan Laboratorium ]::", FileName, param); 
                            }
                        } catch (Exception e) {
                            System.out.println("Notif : "+e);
                        } finally{
    ...
    private boolean uploadPDFToHybrid(String FileName, String docpath, boolean showMessage) {
        try {
            ...
            // Ambil nilai no_rawat dari kolom ke-0
            String no_rawat = tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim();
            ...
            String kodeberkas = Sequel.cariIsi("SELECT kode FROM master_berkas_digital WHERE nama LIKE '%LAB%'");
    ...
    private int resolveMasterRowIndex() {
        int row = tbDokter.getSelectedRow();
        while (row >= 0) {
            Object value = tbDokter.getValueAt(row, 0);
            String noRawat = value == null ? "" : value.toString().trim();
            if (!noRawat.isEmpty() && !">>".equals(noRawat)) {
                return row;
            }
            row--;
        }
    ...
    private int resolveMasterRowIndexFrom(int row) {
        int idx = row;
        while (idx >= 0) {
            Object value = tbDokter.getValueAt(idx, 0);
            String noRawat = value == null ? "" : value.toString().trim();
            if (!noRawat.isEmpty() && !">>".equals(noRawat)) {
                return idx;
            }
    ...
    private String buildLabFileName(String noRawat, String noRm, String tglPeriksa, String jamPeriksa) {
        return "LAB_" + sanitizeFilePart(noRawat) + "_" + sanitizeFilePart(noRm) + "_" + sanitizeFilePart(tglPeriksa) + "_" + sanitizeFilePart(jamPeriksa);
    }

    private String buildLabFileUrl(String fileName, String docpath) {
    ...
    private String buildLabWaMessage(String nama, String noRm, String noRawat, String tglPeriksa, String jamPeriksa, String dokterPerujuk, String dokterPj) {
        ...
        String ruangan = resolveLabRuangan(noRawat);
        StringBuilder pesan = new StringBuilder();
        pesan.append("\uD83D\uDCF2 ").append(header);
        if (!alamatRs.isEmpty()) {
            pesan.append("\n\uD83D\uDCCD ").append(alamatRs);
        }
        pesan.append("\n\n\uD83E\uDDEA HASIL PEMERIKSAAN LABORATORIUM PATOLOGI KLINIS");
        pesan.append("\n\uD83D\uDC64 NAMA : ").append(safeDash(nama));
        pesan.append("\n\uD83C\uDD94 NO.RM : ").append(safeDash(noRm));
        pesan.append("\n\uD83D\uDCCB NO.RAWAT : ").append(safeDash(noRawat));
        pesan.append("\n\uD83D\uDECF RUANGAN : ").append(ruangan);
        pesan.append("\n\uD83E\uDE7A DOKTER PERUJUK : ").append(safeDash(dokterPerujuk));
        pesan.append("\n\uD83D\uDC68\u200D\u2695\uFE0F PENANGGUNG JAWAB : ").append(safeDash(dokterPj));
        pesan.append("\n\uD83D\uDCC5 TGL PERIKSA : ").append(safeDash(tglPeriksa)).append(" ").append(safeDash(jamPeriksa));
        pesan.append("\n\n\uD83D\uDCCE File hasil laboratorium terlampir.");
        return pesan.toString();
    }

    private String resolveLabRuangan(String noRawat) {
        String rawat = safeTrim(noRawat);
        if (rawat.isEmpty()) {
            return "-";
        }
        ...
        if (kamar != null && !kamar.trim().isEmpty()) {
            String namaBangsal = Sequel.cariIsi(
                    "select bangsal.nm_bangsal from bangsal inner join kamar on bangsal.kd_bangsal=kamar.kd_bangsal where kamar.kd_kamar=?",
                    kamar
            );
            String detail = kamar.trim();
            if (namaBangsal != null && !namaBangsal.trim().isEmpty()) {
                detail = detail + ", " + namaBangsal.trim();
            }
            return "Kamar : " + detail;
        }
    ...
    private String resolveManualWaTarget(String fallback) {
        String manual = NoWAManual == null ? "" : NoWAManual.getText();
        if (manual != null) {
