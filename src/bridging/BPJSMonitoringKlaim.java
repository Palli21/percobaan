...
public class BPJSMonitoringKlaim extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        BtnAmbilLokal = new widget.Button();
        ...
        BtnAmbilLokal.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/download24.png"))); // NOI18N
        BtnAmbilLokal.setMnemonic('U');
        BtnAmbilLokal.setText("Ambil Ke Lokal");
        BtnAmbilLokal.setToolTipText("Alt+U");
        BtnAmbilLokal.setName("BtnAmbilLokal"); // NOI18N
        BtnAmbilLokal.setPreferredSize(new java.awt.Dimension(140, 30));
        BtnAmbilLokal.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                BtnAmbilLokalActionPerformed(evt);
            }
        });
        BtnAmbilLokal.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                BtnAmbilLokalKeyPressed(evt);
            }
        });
        panelisi1.add(BtnAmbilLokal);
...
*/
    ...
    private void BtnAmbilLokalActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnAmbilLokalActionPerformed
        int jawab = JOptionPane.showConfirmDialog(null,
            "Proses ini akan menarik data umpan balik klaim (status verifikasi, tarif RS, tarif Grouper, dll) selama 7 bulan terakhir dan memakan waktu cukup lama.\nApakah Anda yakin ingin melanjutkan?",
            "Konfirmasi", JOptionPane.YES_NO_OPTION);
            
        if (jawab == JOptionPane.YES_OPTION) {
            BtnAmbilLokal.setEnabled(false);
            BtnAmbilLokal.setText("Proses...");
            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));

            Thread threadAplikasi = new Thread(new Runnable() {
                @Override
                public void run() {
                    tampilAmbilKeLokal();
                    BtnAmbilLokal.setEnabled(true);
                    BtnAmbilLokal.setText("Ambil Ke Lokal");
                    BPJSMonitoringKlaim.this.setCursor(Cursor.getDefaultCursor());
                    JOptionPane.showMessageDialog(null, "Proses penarikan data Umpan Balik BPJS selesai!");
                }
            });
            threadAplikasi.start();
        }
    }//GEN-LAST:event_BtnAmbilLokalActionPerformed

    private void BtnAmbilLokalKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnAmbilLokalKeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_SPACE){
            BtnAmbilLokalActionPerformed(null);
        }else{
            Valid.pindah(evt,BtnPrint,BtnKeluar);
        }
    }//GEN-LAST:event_BtnAmbilLokalKeyPressed
    ...
    */
    ...
    private widget.Button BtnAmbilLokal;
    ...
    private String formatJenisPelayanan(String kode){
        if("1".equals(kode)){
            return "1. Ranap";
    ...
    private String formatKelasRawat(String kode){
        if("1".equals(kode)){
            return "1. Kelas 1";
        }else if("2".equals(kode)){
            return "2. Kelas 2";
        }else if("3".equals(kode)){
            return "3. Kelas 3";
        }
        return kode==null?"":kode;
    }

    private String nilai(String data){
    ...
    private boolean sesuaiFilter(ResultSet data,JsonNode list) throws Exception{
        ...
        if(!KdPenyakit.getText().trim().equals("") &&
                !KdPenyakit.getText().trim().equalsIgnoreCase(nilai(data.getString("diagawal")))){
            return false;
        }

        if(!KdPoli.getText().trim().equals("") &&
                !KdPoli.getText().trim().equalsIgnoreCase(nilai(data.getString("kdpolitujuan")))){
            return false;
        }

        if(!Kelas.getSelectedItem().toString().equals("Semua") &&
                !Kelas.getSelectedItem().toString().substring(0,1).equals(nilai(data.getString("klsrawat")))){
            return false;
        }
        ...
        if(!TCari.getText().trim().equals("")){
            ...
            String gabungData=(
                    nilai(data.getString("no_sep"))+" "+
                    nilai(data.getString("no_rawat"))+" "+
                    nilai(data.getString("nomr"))+" "+
                    nilai(data.getString("nama_pasien"))+" "+
                    nilai(data.getString("tglsep"))+" "+
                    nilai(data.getString("tglrujukan"))+" "+
                    nilai(data.getString("no_rujukan"))+" "+
                    nilai(data.getString("kdppkrujukan"))+" "+
                    nilai(data.getString("nmppkrujukan"))+" "+
                    nilai(data.getString("diagawal"))+" "+
    ...
    private void tambahDataKlaim(ResultSet data,JsonNode list) throws Exception{
        ...
        for(int i = 0; i < tabMode.getRowCount(); i++) {
            if(tabMode.getValueAt(i, 0).toString().equals(no_sep)) {
                return;
            }
        }

        double pengajuan=list.path("biaya").path("byPengajuan").asDouble();
        double disetujui=list.path("biaya").path("bySetujui").asDouble();
        double tarifGruper=list.path("biaya").path("byTarifGruper").asDouble();
        double tarifRS=list.path("biaya").path("byTarifRS").asDouble();
        double topup=list.path("biaya").path("byTopup").asDouble();

        tabMode.addRow(new Object[]{
            no_sep,nilai(data.getString("no_rawat")),nilai(data.getString("nomr")),nilai(data.getString("nama_pasien")),
            nilai(data.getString("tglsep")),nilai(data.getString("tglrujukan")),nilai(data.getString("no_rujukan")),nilai(data.getString("kdppkrujukan")),
            nilai(data.getString("nmppkrujukan")),nilai(data.getString("kdppkpelayanan")),nilai(data.getString("nmppkpelayanan")),formatJenisPelayanan(data.getString("jnspelayanan")),
            nilai(data.getString("catatan")),nilai(data.getString("diagawal")),nilai(data.getString("diagawal"))+" "+nilai(data.getString("nmdiagnosaawal")),nilai(data.getString("kdpolitujuan")),
            nilai(data.getString("nmpolitujuan")),formatKelasRawat(data.getString("klsrawat")),nilai(data.getString("lakalantas")),nilai(data.getString("keterangankkl")),
            nilai(data.getString("user")),nilai(data.getString("tanggal_lahir")),nilai(data.getString("peserta")),nilai(data.getString("jkel")),
            nilai(data.getString("no_kartu")),nilai(data.getString("tglpulang")),nilai(data.getString("asal_rujukan")),nilai(data.getString("eksekutif")),
            nilai(data.getString("cob")),nilai(data.getString("pembiayaan")),nilai(data.getString("notelep")),
            list.path("Inacbg").path("kode").asText()+" "+list.path("Inacbg").path("nama").asText(),
            list.path("status").asText(),list.path("noFPK").asText(),
            Valid.SetAngka(pengajuan),
            Valid.SetAngka(disetujui),
                ...
                if(response.path("klaim").isArray()){
                    for(JsonNode list:response.path("klaim")){
                        ...
                        try {
                            ...
                            while(rssep.next()){
                                if(sesuaiFilter(rssep,list)){
                                    tambahDataKlaim(rssep,list);
                                }
                            }
                        } catch (Exception e) {
    ...
    public void tampilAmbilKeLokal() {
        try {
            ps = koneksi.prepareStatement(
                    "WITH RECURSIVE date_range AS (\n"
                    + "    SELECT CURDATE() AS the_date\n"
                    + "    UNION ALL\n"
                    + "    SELECT DATE_SUB(the_date, INTERVAL 1 DAY) \n"
                    + "    FROM date_range \n"
                    + "    WHERE the_date > DATE_SUB(CURDATE(), INTERVAL 7 MONTH)\n"
                    + ")\n"
                    + "SELECT * FROM date_range ORDER BY the_date ASC");
            try {
                rs = ps.executeQuery();
                while (rs.next()) {
                    MonitorUmpanBalik(rs.getString("the_date"), "1", "1");
                    MonitorUmpanBalik(rs.getString("the_date"), "1", "2");
                    MonitorUmpanBalik(rs.getString("the_date"), "1", "3");
                    MonitorUmpanBalik(rs.getString("the_date"), "2", "1");
                    MonitorUmpanBalik(rs.getString("the_date"), "2", "2");
                    MonitorUmpanBalik(rs.getString("the_date"), "2", "3");
                }
            } catch (Exception e) {
    ...
    private void MonitorUmpanBalik(String tanggal, String jenispelayanan, String status) {
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        try {
            URL = koneksiDB.URLAPIBPJS() + "/Monitoring/Klaim/Tanggal/" + tanggal + "/JnsPelayanan/" + jenispelayanan + "/Status/" + status;
            System.out.println(URL);
            headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_FORM_URLENCODED);
            headers.add("X-Cons-ID", koneksiDB.CONSIDAPIBPJS());
            ...
            System.out.println("code : " + nameNode.path("code").asText());
            System.out.println("message : " + nameNode.path("message").asText());
//            lblStatus.setText("Mengambil data tanggal : " + tanggal);
            if (nameNode.path("code").asText().equals("200")) {
                response = mapper.readTree(api.Decrypt(root.path("response").asText(), utc));
                
                if (response.path("klaim").isArray()) {
                    for (JsonNode list : response.path("klaim")) {
                        if (Sequel.cariInteger("select count(inacbg_klaim_umpanbalik.no_sep) from inacbg_klaim_umpanbalik where inacbg_klaim_umpanbalik.no_sep='" + list.path("noSEP").asText() + "'") > 0) {
                            Sequel.mengedittf("inacbg_klaim_umpanbalik", "no_sep=?", "kode=?,nama=?,bypengajuan=?,bydisetujui=?,bytarifgrouper=?,bytarifrs=?,bytopup=?,nofpk=?,status=?", 10, new String[]{
                                list.path("Inacbg").path("kode").asText(),
                                list.path("Inacbg").path("nama").asText(),
                                list.path("biaya").path("byPengajuan").asDouble() + "",
                                list.path("biaya").path("bySetujui").asDouble() + "",
                                list.path("biaya").path("byTarifGruper").asDouble() + "",
                            ...
                            System.out.println("Berhasil update ke lokal" + list.path("noSEP").asText());
                        } else {
                            if (Sequel.menyimpantf("inacbg_klaim_umpanbalik", "?,?,?,?,?,?,?,?,?,?", "UMPAN BALIK KALIM BPJS", 10, new String[]{
                                list.path("noSEP").asText(),
                                list.path("Inacbg").path("kode").asText(),
                                list.path("Inacbg").path("nama").asText(),
                                list.path("biaya").path("byPengajuan").asDouble() + "",
                                ...
                                System.out.println("Berhasil menyimpan ke lokal" + list.path("noSEP").asText());
                            }
                        }
                    }
                }
