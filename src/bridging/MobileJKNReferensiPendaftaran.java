                    ...
                    while(rs.next()){
                        try {
                            ...
                            if(nameNode.path("code").asText().equals("200")||nameNode.path("code").asText().equals("208")){
                                ...
                                Sequel.mengedittf("referensi_mobilejkn_bpjs","nobooking=?","status='Checkin',validasi=now(),statuskirim='Sudah'",1,new String[]{rs.getString("nobooking")});
                                Sequel.queryu("update reg_periksa set jam_reg=current_time() where no_rawat='"+rs.getString("no_rawat")+"'");
                            }
                            System.out.println("respon WS BPJS Kirim Referensi JKN : "+nameNode.path("code").asText()+" "+nameNode.path("message").asText()+"\n");
                        }catch (Exception ex) {
                            System.out.println("Notifikasi Bridging : "+ex);
